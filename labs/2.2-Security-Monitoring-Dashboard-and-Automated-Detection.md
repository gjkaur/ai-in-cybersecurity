...
![Lab 2.2 detection pipeline — log groups through metric filters, alarms, and AI Operations](images/lab-2.2-detection-pipeline.png)
...
For each widget below, click **Add widget** and follow the correct path in the next section. When building each widget, **do not enable "Persist time range"** — leave it off so the dashboard time range control applies globally.

### How to add a widget (read this first)

The **Add widget** dialog is a two-step wizard. Screen 1 asks for **Data type** and **Widget type**; screen 2 is where you pick the metric or paste the Logs Insights query.

![Add widget dialog — select Metrics data type and Line chart widget type](images/new-widget.png)

**The most common mistake:** leaving **Data type** on **Metrics** when the widget needs a **Logs Insights query**. Widget 1 uses **Metrics**; Widgets 2–7 use **Logs**.

| Widget | Data type (screen 1) | Widget type (screen 1) | Log group (screen 2) |
|--------|----------------------|------------------------|----------------------|
| 1 — Web Scan Count | **Metrics** | **Line** | — (pick metric instead) |
| 2, 4, 5, 6 — log tables | **Logs** | **Logs table** | `/soc-lab/nginx` |
| 3 — Top Scanned Paths | **Logs** | **Bar** | `/soc-lab/nginx` |
| 7 — AI exploration | **Logs** | your choice | `/soc-lab/nginx` |
| Part 5 — Sudoers | **Logs** | **Logs table** | `/soc-lab/audit` |

#### Path A — Metric widget (Widget 1 only)

1. On the dashboard, click **Add widget**.
2. Leave **CloudWatch** selected in the left sidebar.
3. Under **Data type**, click **Metrics**.
4. Under **Preferred experience**, leave **Metrics console** selected.
5. Under **Widget type**, click **Line**, then click **Next**.
6. In the metric browser, open **SOCLab** → select **WebScanAttempts** → click **Create widget**.
7. On the widget configuration screen, set:
   - **Statistic**: **Sum**
   - **Period**: **5 minutes**
   - **Title**: `Web Scan Count`
8. Confirm **Persist time range** is **off**, then click **Save** (or **Save dashboard**).

![Graphed metrics — Sum statistic and 5-minute period](images/graphed-metrics.png)

#### Path B — Logs Insights widget (Widgets 2–7 and Part 5)

1. On the dashboard, click **Add widget**.
2. Leave **CloudWatch** selected in the left sidebar.
3. Under **Data type**, click **Logs** — this is required; do not stay on Metrics.
4. Under **Widget type**, click **Logs table** (or **Bar** for Widget 3), then click **Next**.

   ![Add widget dialog — select Logs data type and Logs table widget type](images/log-widget.png)

5. On the next screen, under **Select log groups**, choose the log group for this widget (usually `/soc-lab/nginx`).
6. Paste the query from the widget instructions below into the query editor.
7. Click **Run query** to verify results appear, then set the **Title** for the widget.
8. Confirm **Persist time range** is **off**, then click **Create widget** (or **Save dashboard**).

> **Tip:** If you only see a metric picker and no query box, you selected **Metrics** on screen 1. Click **Cancel**, start again, and choose **Logs**.

#### Troubleshooting

| Problem | Fix |
|---------|-----|
| No query editor, only a metric browser | **Data type** is **Metrics** — cancel and restart with **Logs** |
| Widget shows "No data" | Set dashboard time range to **Last 1 hour**; re-run the Part 2 curl probe |
| Metric widget flat at zero | Wait 2–5 minutes after probe; confirm metric filter `nginx-scan-attempts` exists |
| Bar chart empty (Widget 3) | Confirm probe generated 404s; run the query in **Logs Insights** first to test |

---

### Widget 1 — Web Scan Count (Line Chart)

Use **Path A** (Metrics → Line) from above.

In the metric browser, select **SOCLab** → **WebScanAttempts**.
...
Use **Path B** (Logs → Logs table). Log group: `/soc-lab/nginx`
...
Use **Path B** (Logs → **Bar**). Log group: `/soc-lab/nginx`
...
Use **Path B** (Logs → your choice of chart type). Log group: `/soc-lab/nginx`
...
2. Use **Path B** (Logs → Logs table). Log group: `/soc-lab/audit`. Query:
