...
**Instructors — quota increases:** AWS Console → **Service Quotas** → **Amazon Bedrock** → region **US West (Oregon)**. Search for **Mistral Large** quotas, especially **on-demand tokens per day** (the limit behind `ThrottlingException: Too many tokens per day`). Training accounts share this quota across all students. For ~25 students, request at least **500,000 tokens/day** in `us-west-2` for `mistral.mistral-large-2402-v1:0`.

Verify access: **Bedrock → Model catalog** → search **Mistral Large** → confirm **us-west-2**. Test in **Chat/Text playground** with the same model before class.
