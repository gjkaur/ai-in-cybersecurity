...
![Lab 1.2 flow — CloudShell attacks, EC2 log collection, and Bedrock AI triage](/labs/images/lab-1.2-attack-triage-flow.svg)
...
| `ThrottlingException: Too many tokens per day` | Account-wide Bedrock daily token quota exhausted | Wait ~24 hours, ask your instructor to raise quotas, or use the fallback below |
...
You simulated a multi-vector attack — credential brute force, web reconnaissance, and privilege escalation persistence — captured it with auditd and nginx, and triaged it with an AI SOC analyst (Bedrock) or a manual analysis when quota limits apply. The same logs that Claude analyzes will be streamed to CloudWatch in Day 2, where automated detection pipelines will surface the same events without any manual querying.
